...
vi.mock("../api", async () => {
  ...
  return {
    dashboardApi: {
      ...
      getRecentPolicies: vi
        .fn()
        .mockResolvedValue(mockData.mockPolicies.slice(0, 3)),
      getRecentClaims: vi
        .fn()
        .mockResolvedValue(mockData.mockClaims.slice(0, 3)),
    },
  };
});
...
describe("StatsCards", () => {
  ...
  it("displays correct stat values", () => {
    ...
    expect(
      screen.getAllByText(mockDashboardStats.pendingClaims.toString()).length,
    ).toBeGreaterThan(0);
    expect(
      screen.getAllByText(mockDashboardStats.approvedClaims.toString()).length,
    ).toBeGreaterThan(0);
    expect(
      screen.getByText(mockDashboardStats.nextPaymentAmount),
    ).toBeInTheDocument();
  });
...
describe("AvailablePolicies", () => {
  it("renders loading skeletons when isLoading is true", () => {
    ...
    expect(
      screen.getByTestId("available-policies-loading"),
    ).toBeInTheDocument();
  });
